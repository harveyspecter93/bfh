...
public class BasketManager {
	/*
	 * Gruppenarbeit 01: Warenkorb Klasse 1o Ziegler, Andrin; Frei, Yannick;
	 * Dr�yer, Michael
	 *
	 */
	...
	public void showCurrentBasket() {
		...
		if (itemsInBasket.isEmpty()) {
			...
			System.out.println(this.bundle.getString("TOTAL_IS") + String.format("%.2f", totalPrice) + " "
					+ this.bundle.getString("WITH_TAX") + String.format("%.2f", totalTax) + " "
					+ this.bundle.getString("TAXES_CHF")
					+ String.format("%.2f", calculateTotalTax(totalPrice, totalTax)) + "%). \n");
	...
	public void enableBuying() {
		...
		System.out.println(this.bundle.getString("HOW_MANY") + itemToBuy.getName() + " "
				+ this.bundle.getString("CHOICE_AMOUNT_ADD"));
		// check for Quit
	...
	public void addItemToBasket(ShoppingItem shoppingItem, int amount) {
		...
		if (itemsInBasket.isEmpty()) {
			...
			System.out.println(
					this.bundle.getString("ITEM") + shoppingItem.getName() + " " + this.bundle.getString("WAS_ADDED"));
		} else {
			...
			if (itemInBasket instanceof ShoppingItem) {
				...
				itemsInBasket.add(shoppingItem);
				shoppingItem.setAmountInBasket(amount);
				System.out.println(this.bundle.getString("ITEM") + shoppingItem.getName() + " "
						+ this.bundle.getString("WAS_ADDED"));
			}
		}
